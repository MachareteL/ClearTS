...
type cartType = {
  cartState: boolean;
  setCartStateFalse: () => void;
};

export default function Cart({ cartState, setCartStateFalse }: cartType) {
  ...
  useEffect(() => {
    rota.events.on("routeChangeComplete", setCartStateFalse);
  }, [rota]);
  ...
  return (
    <Transition.Root show={cartState} as={Fragment}>
      <Dialog as="div" className="relative z-10" onClose={setCartStateFalse}>
        <Transition.Child
          as={Fragment}
